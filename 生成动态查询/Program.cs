...
namespace 生成动态查询
{
    internal class Program
    {
        private static void Main()
        {
            ...
            foreach (var article in results)
                Console.WriteLine("发布日期:" + article.Published + "，浏览数：" + article.Views + "");
            Console.ReadLine();
        }
    }
    ...
    public class Extention<T>
    {
        public static IQueryable<T> MySearchList(IQueryable<T> myArticleTable, T myArticle)
        {
            ...
            Expression right = Expression.Property(Expression.Constant(myArticle), typeof (T).GetProperty("Published"));
                //访问属性的表达式
            ...
            Expression right2 = Expression.Property(Expression.Constant(myArticle), typeof (T).GetProperty("Views"));
                //访问属性的表达式
